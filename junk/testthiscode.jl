...
using LinearAlgebra
using Plots;
using Flux, DiffEqFlux, DiffEqOperators
#set up variables
...
true_sol = solve(generate_data, ROCK2(), saveat=t)
...
ff = ODEFunction(dudt, jac_prototype = JacVecOperator(dudt, u0))
current_model = ODEProblem(ff, u0, tspan, inputs)
param_u = param(u0)
#Loss function
function modeled_ode()
    diffeq_adjoint(inputs, current_model, Tsit5(); u0 = param_u, saveat = t, backsolve=false)
end
modeled_ode()
...
@time Flux.train!(loss_funct, lyrs, iter, opt, cb = cb )
